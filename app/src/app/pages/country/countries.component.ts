...
export class CountriesComponent {
  ...
  filterConfigs: ColumnFilterConfig[] = [
    { field: 'isoKey', type: 'string', label: 'ISO Key' },
  ...
  loadCountries(): void {
    ...
    this.countriesService.getCountries(this.page(), this.pageSize(), this.sortBy(), this.sortDirection(), queryString)
      .subscribe({
        next: (data: any) => {
          this.countries.set(data.items ?? []);
          const info = data.$info?.paging;
          this.totalItems.set(info?.['total-items'] ?? 0);
          this.totalPages.set(info?.['total-pages'] ?? 0);
        },
        error: (err: any) => { this.countries.set([]); }
      });
  }
  ...
  toggleSelection(isoKey: string): void {
    const selected = new Set(this.selectedCountries());
    if (selected.has(isoKey)) selected.delete(isoKey); else selected.add(isoKey);
    this.selectedCountries.set(selected);
  }

  toggleAllSelection(checked: boolean): void {
    const selected = new Set<string>();
    if (checked) this.countries().forEach(country => selected.add(country.isoKey));
    this.selectedCountries.set(selected);
  }

  getLocalizedName(country: Country): string {
    return country.name?.[this.lang()] || country.name?.['en'] || country.isoKey;
  }

  deleteSelected(): void {
    if (confirm(this.transloco.translate('common.messages.confirmDeleteMultiple', { count: this.selectedCountries().size }))) {
      const isoKeys = Array.from(this.selectedCountries());
      this.countriesService.bulkDeleteCountries(isoKeys).subscribe({
        next: () => { this.selectedCountries.set(new Set()); this.deleteError.set(null); this.loadCountries(); },
        error: () => { this.deleteError.set(this.transloco.translate('common.errors.country.deleteError')); this.loadCountries(); }
      });
    }
  }
}
